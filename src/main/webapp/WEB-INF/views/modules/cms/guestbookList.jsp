...
<body>
	...
	<form:form id="searchForm" modelAttribute="guestbook"
		action="${ctx}/cms/guestbook/" method="post"
		...
		<form:select id="type" path="type" class="input-small">
			...
			<form:options items="${fns:getDictList('cms_guestbook')}"
		...
		<tbody>
			<c:forEach items="${page.list}" var="guestbook">
				<tr>
					...
					<td><fmt:formatDate value="${guestbook.reDate}"
							pattern="yyyy-MM-dd HH:mm:ss" /></td>
					<shiro:hasPermission name="cms:guestbook:edit">
						<td><c:if test="${guestbook.delFlag ne '2'}">
								<a

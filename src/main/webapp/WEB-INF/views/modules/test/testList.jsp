...
<body>
	<ul class="nav nav-tabs">
		...
		<shiro:hasPermission name="test:test:edit">
			<li><a href="${ctx}/test/test/form">组件表单</a></li>
		</shiro:hasPermission>
	</ul>
	<div class="btn-toolbar breadcrumb">
		<div class="btn-group">
			...
			<a class="btn" href="javascript:dele();"
				onclick="return confirmx('确认要删除该测试吗？', this.href)"><i
				class="icon-remove"></i>删除</a>
		</div>
		<div class="btn-group">
			<a class="btn" onclick="$('#searchForm').toggle();su.autoHeight();"><i
				class="icon-search"></i>查询</a>
		</div>
	</div>
	<form:form id="searchForm" modelAttribute="test"
		action="${ctx}/test/test/listData.json" method="post"
		class="breadcrumb form-search hide" onsubmit="return page();">

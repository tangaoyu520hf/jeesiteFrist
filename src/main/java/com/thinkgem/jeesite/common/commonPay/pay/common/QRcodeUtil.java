...
/*	private BufferedImage qRCodeCommon(String content, String imgType, int size,String ccbPath) {
		...
		try {
			...
			//二维码中间图标
			if(ccbPath!=null&&!"".equals(ccbPath)){
			Image img = ImageIO.read(new File(ccbPath));//实例化一个Image对象。
            gs.drawImage(img, imgSize/2-20, imgSize/2-20,40,40, null);
			}
			gs.dispose();
			bufImg.flush();
		} catch (Exception e) {
...
/**
	...
	public static void main(String[] args) {
		...
		String encoderContent = "测试测试";
        QRcodeUtil handler = new QRcodeUtil();
		handler.encoderQRCode(encoderContent, imgPath, "png",9,"d:/123.jpg");
//		try {
